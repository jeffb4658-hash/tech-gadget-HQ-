...
TOKEN_SAFETY_PAD  = 8_000   # stop generating when this many tokens remain
MAX_RETRIES       = 4       # retries per groq_call on 429
_tokens_used      = 0       # running tally for this process
...
CATEGORIES = {
    ...
    "home-office":   {"label": "Home Office",   "emoji": "💻", "color": "#00bfff"},
    "car-tech":      {"label": "Car Tech",      "emoji": "🚗", "color": "#ff4500"},
    "fitness-tech":  {"label": "Fitness Tech",  "emoji": "💪", "color": "#39ff14"},
}
...
PRODUCT_SEEDS = {
    "smart-home": [
        ...
        "smart smoke detector", "smart irrigation controller",
    ],
    "edc": [
    ...
    "outdoor-tech": [
        ...
        "trekking poles", "portable water filter",
    ],
    "survival-tech": [
        ...
        "mylar emergency blanket", "freeze dried food kit",
    ],
    "audio-video": [
        ...
        "noise cancelling headphones", "portable projector",
    ],
    "home-office": [
        "standing desk converter", "ergonomic chair", "monitor arm",
        "webcam", "mechanical keyboard", "wireless mouse",
        "USB-C hub", "desk lamp", "laptop stand",
        "blue light glasses", "cable management kit",
    ],
    "car-tech": [
        "dash cam", "OBD2 scanner", "car jump starter",
        "tire inflator", "car phone mount", "backup camera",
        "car vacuum", "GPS navigator", "car air purifier",
        "blind spot mirror", "car battery charger",
    ],
    "fitness-tech": [
        "fitness tracker", "smart scale", "heart rate monitor",
        "resistance bands set", "foam roller", "pull up bar",
        "jump rope", "GPS running watch", "massage gun",
        "smart water bottle", "ab wheel",
    ],
}

# ── GROQ CALL (with retry + budget guard) ─────────────────────────────────────
def groq_call(prompt: str, max_tokens: int = 1200) -> str:
    global _tokens_used

    # Budget guard: bail cleanly before hitting the wall
    estimated_prompt_tokens = len(prompt) // 4  # rough estimate
    if _tokens_used + estimated_prompt_tokens + max_tokens + 500 > DAILY_TOKEN_LIMIT - TOKEN_SAFETY_PAD:
        print(f"[budget] TPD ceiling approaching ({_tokens_used:,} used). Stopping cleanly.")
        sys.exit(0)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            used = resp.usage.total_tokens if resp.usage else (max_tokens + estimated_prompt_tokens)
            _tokens_used += used
            print(f"[tokens] +{used} this call | {_tokens_used:,}/{DAILY_TOKEN_LIMIT:,} today")
            return resp.choices[0].message.content.strip()

        except groq_module.RateLimitError as e:
            msg = str(e)
            # Parse "Please try again in Xm Ys" from error body
            match = re.search(r'in (\d+)m([\d.]+)s', msg)
            if match:
            ...
            if attempt < MAX_RETRIES - 1:
                print(f"[rate_limit] 429 hit. Sleeping {wait:.0f}s (attempt {attempt + 1}/{MAX_RETRIES})")
                time.sleep(wait)
            else:
                print(f"[rate_limit] Max retries exhausted. Stopping cleanly.")
                sys.exit(0)
...
def main():
    ...
    for i in range(POSTS_PER_RUN):
        ...
        available = [s for s in seeds if s not in used_topics] or seeds
    ...
    generate_sitemap()
    generate_feed()
    print(f"[done] {posts_generated} posts generated | {_tokens_used:,} tokens used today")
